...
function createForbidTest(promise, successMessage, failureMessage, extraMessage) {
  async();
  extraMessage = extraMessage ? ' - ' + extraMessage : '';
  return promise
    .then(handleFailure(failureMessage + extraMessage), handleSuccess(successMessage + extraMessage))
    .finally(asyncDone);
}
...
function forbidUpdate(name, source, data, message) {
...
function forbidDelete(name, source, message) {
  return createForbidTest(
    fetchItem.call(this, name, source)
      .then(function(item) {
        return item.destroyRecord();
...
function forbidCreate(name, data, message) {
  return createForbidTest(
    createItem.call(this, name, data),
...
function forbidList(name, message) {
  return createForbidTest(
    getList.call(this, name),
...
function forbidGet(name, source, message) {
  return createForbidTest(
    fetchItem.call(this, name, source),
